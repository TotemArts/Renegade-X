...
struct native EmsFile
{
	...
	/** File size */
	var int FileSize;
	/** The date the file was last uploaded to the backend */
	var init String UploadedDateTime;

	structcpptext
	{
		// Serialization
		friend FArchive& operator<<(FArchive& Ar, FEmsFile& EmsFile)
		{
			Ar << EmsFile.Hash << EmsFile.DLName << EmsFile.Filename << EmsFile.FileSize << EmsFile.UploadedDateTime;
			return Ar;
		}
	}
};
