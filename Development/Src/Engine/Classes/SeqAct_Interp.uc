...
/** Stop the current Matinee when a new Matinee starts if both have Director tracks */
var(CameraTransitions)	bool	bAutoStopWhenDirectorChanges;

/** When using a camera Transition Time on the very first frame, should we lock the old camera before blending or keep updating it during the blend? */
var(CameraTransitions)	bool	bLockOutgoingCameraOnFirstTransition;

/** When using a camera Transition Time on the very first frame, ignore the transition if it is coming from a player or pawn instead of another camera */
var(CameraTransitions)	bool	bSkipFirstTransitionIfNotFromCamera;
...
defaultproperties
{
	...
	ConstantCameraAnim=0
	ConstantCameraAnimRate=4.0

	`if(`isdefined(CHAIR_INTERNAL))
		// The default behavior for UDK is false, but we always want this true for ChAIR games
		bLockOutgoingCameraOnFirstTransition=true
		bSkipFirstTransitionIfNotFromCamera=true
		bAutoStopWhenDirectorChanges=true
	`endif
}
