...
function bool RepairCloseVehicles() 
{
	...
	if(bShouldRepVehs) {
		for ( V=UTGame(WorldInfo.Game).VehicleList; V!=None; V=V.NextVehicle )
		{
			...
			if(bDefending 
				&& VSize(V.location - CurrentBO.GetShootTarget(self).location) > 2500 ) {
				continue;
			}
...
function bool FindInfiltrationPath()
{
	...
	if (VSize(CurrentBO.myBuilding.GetMCT().location - Pawn.location) < 500
...
function bool FindVehicleAssaultPath()
{
	...
	BestPath = FindPathToward(BuildingPoint,true);
...
Begin:
	...
	else if(PlayerStart(RouteGoal) != None && PTTask != "")
	{
		...
		if(Vehicle(Pawn) != None && Enemy == None && VSize(RouteGoal.Location - Pawn.Location) <= 750)
			LeaveVehicle(true);
	}
