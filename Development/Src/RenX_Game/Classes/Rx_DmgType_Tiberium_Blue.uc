class Rx_DmgType_Tiberium_Blue extends Rx_DmgType_Special;
...
{
	local Rx_Emit_TiberiumDmg_Blue BF;
	local UTPawn UTP;

	UTP = UTPawn(P);
	if (UTP != None && !UTP.bGibbed) 
	{
		BF = P.Spawn(class'Rx_Emit_TiberiumDmg_Blue',P,, P.Location, rotator(Momentum));
...
{
	// FIXME:: 
	GibTrail=ParticleSystem'RX_WP_ChemicalThrower.Effects.FX_BlueTibDamage'

	bPiercesArmor=true
	bCausesBleed=true
	BleedType=class'Rx_DmgType_TiberiumBleed_Blue'
