...
simulated function DrawAttackT(Pawn P, byte TNumber, float InitialTime)
{
	...
	if(Rx_Vehicle(P) != none )
	{
		...
		if((Secs-(InitialTime+15)) > 0) IconScale = fmax(0.1, 1.25-(0.20*(Secs-(InitialTime+15)) ));
		else
		IconScale=1.25;
	}	
	...
	{
		MinFadeAlpha=100;	
		if((Secs-(InitialTime+15)) > 0) IconScale = fmax(0.1, 1.0-(0.20*(Secs-(InitialTime+15)))) ;
		else
		IconScale=1.0;
	}
