...
function string ParsePlayerTokens(string format, Rx_PRI PRI)
{
	...
	ParseStringIntoArray(format, tokens, " ", true);
	if (tokens.Length == 0)
		return "";

	r = tokens[0] `s class'Rx_Rcon_Command_ClientVarList'.static.ParseTokenPRIi(PRI, tokens[0]);
	for (index = 1; index != tokens.Length; index++)
		r $= `rcon_delim $ tokens[index] `s class'Rx_Rcon_Command_ClientVarList'.static.ParseTokenPRIi(PRI, tokens[index]);
	return r;
}
...
function string trigger(string parameters)
{
	...
	if (pos != -1)
	{
		PRI = Rx_Game(`WorldInfoObject.Game).ParsePlayer(Left(parameters, pos), error);
		if (PRI == None)
			return error;
	...
	else
	{
		...
		{
			parameters $= `rcon_delim $ "vehicle" `s Controller(PRI.Owner).Pawn == None ? "" : Rx_Vehicle(Controller(PRI.Owner).Pawn) == None ? "" : string(Controller(PRI.Owner).Pawn.Class.name);
			if (Rx_Controller(PRI.Owner) != None)
				parameters $= `rcon_delim $ "steam" `s `WorldInfoObject.Game.OnlineSub.UniqueNetIdToString(Rx_Controller(PRI.Owner).PlayerReplicationInfo.UniqueId) `s "ip" `s Rx_Controller(PRI.Owner).GetPlayerNetworkAddress() `s "ping" `s PRI.Ping * 4;
		}
	}
