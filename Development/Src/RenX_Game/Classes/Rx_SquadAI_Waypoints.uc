...
{
	if(SquadObjective != None && VSize(B.location - Rx_BuildingObjective(SquadObjective).InfiltrationPoint.location) 
			> VSize(B.Enemy.location - Rx_BuildingObjective(SquadObjective).InfiltrationPoint.location)) 
	{
		return true;
	}
	return false;
...
function float RateDefensivePosition(NavigationPoint N, UTBot CurrentBot, Actor Center)
{
	...
	if (Dist < 400.0 || (Center.IsA('Rx_BuildingObjective') && Dist < 2000))
	{
		return (0.00025 * Dist * MCTReachMod);
	}
...
function bool ValidEnemy(Pawn NewEnemy)
{
	...
	{					
		...
		{										
			if (VSize(NewEnemy.Location - M.Pawn.Location) <= SpyTrackRadius)
			{
				return true;
			}
		}
