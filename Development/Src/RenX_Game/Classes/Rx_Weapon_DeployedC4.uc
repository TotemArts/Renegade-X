...
event Touch( Actor Other, PrimitiveComponent OtherComp, vector HitLocation, vector HitNormal )
{
   ...
   if (Other != self.Instigator && (Volume(Other) == none || BlockingVolume(Other) != none) && Other.bBlockActors == true)
   {
...
function Explosion()
{
	...
	if ( Role == ROLE_Authority )
	{
		...
		if(ImpactedActor != None && (ImpactedActor.GetTeamNum() != GetTeamNum() || bDamageAll)) {
			ImpactedActor.TakeDamage(Damage,InstigatorController,Location,vect(0,0,0),ChargeDamageType);
		}		
