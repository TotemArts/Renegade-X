...
*
*********************************************************/
class TS_Vehicle_Titan_C extends Rx_Vehicle
    placeable;
...
simulated function PostBeginPlay()
{
	...
	//SetTimer(0.3, true, 'analysePhysics');
}
...
/**
function checkStillEncroached()
{
	local UDKVehicle veh; 
	local bool bNotTouchedByVehicles;

	bNotTouchedByVehicles = true;

	ForEach TouchingActors(class'UDKVehicle',veh)
	{
		if(veh != none) 
		{
			bNotTouchedByVehicles = false;
			loginternal("still touched");
		}
	}
	if(bNotTouchedByVehicles)
	{
		bEncroachingVehicle=None;
	}
	ClearTimer('checkStillEncroached');
}
*/
