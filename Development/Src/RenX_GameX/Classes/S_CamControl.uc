...
//
//-----------------------------------------------------------
class S_CamControl extends actor
		config( RypelCam );
...
function init() {
	...
	local int i;
	local S_Knoten Other;
	foreach DynamicActors( class'S_Knoten', Other ) {
	...
	if ( !conf ) {
		...
		for ( i = 0; i < z; i++ ) {
			Other = Spawn( class'S_Knoten', self, , Flag_Locations[i] );
			if(bForC130) {
				Other.bForC130 = true;	
			}
		}
	...
	foreach DynamicActors( class'S_Knoten', Other ) {
		...
		if ( Other.Location != Flag_Locations[z2] ) {
			if ( !combineLoc ) {
				...
				z3 = 0;
				foreach DynamicActors( class'S_Knoten', Other10 ) {
...
function Timer()
{
	...
	if ( ( draw_Spline == false ) && ( !drawSplineWasFalse ) ) {
		foreach DynamicActors( class'S_Knoten', OtherKnot ) {
