class S_HelipadPurchaseSystem extends S_PurchaseSystem
notplaceable;

var array<S_Building_Helipad_BlackHand> GDIHelipad;
...
simulated event PostBeginPlay()
{
	...
	ForEach AllActors(class'Rx_Building',building)
	{
		...
		else if (RxIfc_FactoryVehicle(building) != None)
		{
			if(building.GetTeamNum() == 0)
			{
				if(S_Building_Helipad_BlackHand(building) != None)
					GDIHelipad.AddItem(S_Building_Helipad_BlackHand(building));
				else
...
simulated function string GetFactoryDescription(byte teamID, string menuName, Rx_Controller rxPC) 
{
	...
	if (menuName == "VEHICLES") {
		if (teamID == TEAM_GDI) {
			factoryName = WeaponsFactory.Length > 0 ? Caps(Actor(WeaponsFactory[0]).GetHumanReadableName()) : "AIRSTRIP";
		...
		if (teamID == TEAM_GDI) {
			factoryName = Barracks.Length > 0 ? Caps(Actor(Barracks[0]).GetHumanReadableName()) : "HAND OF NOD";
		} else if (teamID == TEAM_NOD) {
