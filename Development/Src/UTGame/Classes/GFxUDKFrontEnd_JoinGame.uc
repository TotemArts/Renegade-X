...
function Cleanup()
{
	...
	if (PC != none)
	{
		PC.JoinGameMenu = none;
	}
}

/**
 * Updates the gamemode text on the filter button (does not modify filters)
 */
final function UpdateFilterButton()
{
	...
	FilterGameMode = GameSettings.GetPropertyAsStringByName('CustomGameMode');
...
private final function OnPasswordDialog_OK(GFxClikWidget.EventData ev)
{
	...
	{
		// strip out all
		ServerPassword = static.StripInvalidPasswordCharacters(Password);
		ProcessJoin();
	}
	else
	{
...
{
	// Get the match type based on the platform.
	return class'WorldInfo'.static.IsConsoleBuild(CONSOLE_XBox360) 
...
function UpdateListDataProvider()
{
	...
	if (LatestGameSearch != none)
	{
		...
		for (i = 0; i < LatestGameSearch.Results.Length; i++)
		{
			...
			Ping = LatestGameSearch.Results[i].GameSettings.PingInMs;
			TempObj.SetString("Ping",          (Ping < 0) ? ("-") : (String(Ping)) );
			TempObj.SetString("Map",           LatestGameSearch.Results[i].GameSettings.GetPropertyAsString(PROPERTY_CUSTOMMAPNAME));
...
function OnJoinGameComplete(name SessionName, bool bSuccessful)
{
	...
	local UTGameSettingsCommon CurGameSettings;
	...
	if (GameInterface != None)
	{
		if (bSuccessful)
		{
			...
			{
				if (OnlineGameInterfaceImpl(GameInterface) != none)
				{
					CurGameSettings = UTGameSettingsCommon(OnlineGameInterfaceImpl(GameInterface).GameSettings);
				}

				// If the 'SteamServerId' value for the server isn't empty, then this server is using steam sockets
				if (CurGameSettings != none && CurGameSettings.SteamServerId != "")
				{
...
/**
 * Server browser shortcut keys
 */
function bool OnFilterButtonInput(int ControllerId, name ButtonName, EInputEvent InputEvent)
{
	if (ButtonName == 'LeftControl' || ButtonName == 'RightControl')
	{
		if (InputEvent == IE_Pressed || InputEvent == IE_Repeat)
		{
			bCtrlPressed = True;
		}
	...
	else if (InputEvent == IE_Pressed)
	{
		...
		else if (ButtonName == 'F5')
		{
			...
			//else
			//{
			//	ConditionalRefreshServerList(FakePlayerIndex);
			//}
 ...
 */
final function ForceLANOption()
{
	local int ValueIndex, i;
	...
	// LAN queries show all gametypes, so switch to the 'UTGameSearchCustom' (i.e. 'All Gametypes') search mode
	Class'UTUIDataStore_MenuItems'.static.GetAllResourceDataProviders(Class'UTUIDataProvider_GameModeInfo', Providers);
	...
	for (i=0; i<Providers.Length; ++i)
	{
		...
		if (Provider != none && Provider.GameSearchClass == "UTGameSearchCustom")
		{
			ValueIndex = i;
			break;
		}
	...
	{
		ValueIndex = SearchDataStore.FindSearchConfigurationIndex('UTGameSearchCustom');

		if (ValueIndex != INDEX_None)
		{
			CustomSearch = UTGameSearchCustom(SearchDataStore.GameSearchCfgList[ValueIndex].Search);
...
{
	local int ProviderIdx, i;
	local string GameTypeClassName;
	local UTGameSettingsCommon GameSettings;
	local UIDataStore_OnlineGameSettings SettingsDataStore;
	local UTGameSearchCustom CustomSearch;
	local UTGameSearchCommon CommonSearch;
	local array<UDKUIResourceDataProvider> Providers;
	local UTUIDataProvider_GameModeInfo Provider;
	...
	GameTypeClassName = GameSettings.GetPropertyAsStringByName('CustomGameMode');

	// Initialize the 'CustomGameMode' data store value, if it's not already initialized
	if (GameTypeClassName == "")
	{
		CommonSearch = UTGameSearchCommon(SearchDataStore.GetCurrentGameSearch());

		if (CommonSearch != none && CommonSearch.GameClass != "")
		{
			GameSettings.SetPropertyFromStringByName('CustomGameMode', CommonSearch.GameClass);
			GameTypeClassName = CommonSearch.GameClass;
		}
	}

	if (GameTypeClassName != "")
	{
		if (GameTypeClassName ~= "All Gametypes" || GameTypeClassName ~= "Unlisted Gametypes")
		{
			...
			if (i != INDEX_None)
			{
				...
				// The default state of 'UTGameSearchCustom' is to list all gametypes; only add gametype filters if needed
				if (GameTypeClassName ~= "Unlisted Gametypes")
				{
					if (Class'GameEngine'.static.GetOnlineSubsystem().Class.Name == 'OnlineSubsystemSteamworks')
					{
						// Add filter entries for all known gametypes
						// NOTE: Only stock UDK gametypes, not mod gametypes (@todo JohnB: add mod hook-ins later)
						CustomSearch.ResetFilters();

						CustomSearch.AddANDFilter('DM');
						CustomSearch.AddORFilter('DM', PROPERTY_CUSTOMGAMEMODE, OGSET_Property, OGSCT_NotEquals,
										"UTGame.UTDeathmatch");

						CustomSearch.AddANDFilter('TDM');
						CustomSearch.AddORFilter('TDM', PROPERTY_CUSTOMGAMEMODE, OGSET_Property, OGSCT_NotEquals,
										"UTGame.UTTeamGame");

						CustomSearch.AddANDFilter('CTF');
						CustomSearch.AddORFilter('CTF', PROPERTY_CUSTOMGAMEMODE, OGSET_Property, OGSCT_NotEquals,
										"UTGameContent.UTCTFGame_Content");

						CustomSearch.AddANDFilter('VCTF');
						CustomSearch.AddORFilter('VCTF', PROPERTY_CUSTOMGAMEMODE, OGSET_Property, OGSCT_NotEquals,
										"UTGameContent.UTVehicleCTFGame_Content");

						CustomSearch.CommitFilters();
					}
					else
					{
						`log("The 'Unlisted Gametypes' search only works with OnlineSubsystemSteamworks");
					}
				}
			...
			{
				`log("Error, UTGameSearchCustom is not set in GameSearchCfgList");
			}
		}
		else
		{
			...
			for (i=0; i<Providers.length; i++)
			{
				...
				if (Provider != None && Provider.GameMode == GameTypeClassName && Provider.GameSearchClass != "")
				{
					ProviderIdx = SearchDataStore.FindSearchConfigurationIndex(name(Provider.GameSearchClass));
					break;
				}
			}
			...
			if (ProviderIdx != INDEX_None)
			{
				...
				CommonSearch = UTGameSearchCommon(SearchDataStore.GameSearchCfgList[ProviderIdx].Search);

				if (CommonSearch != none)
				{
					CommonSearch.InitFilters();
				}
			}
		}


		// Assume settings were updated, and kickoff a new search
		if (SearchDataStore.HasOutstandingQueries())
		{
			CancelQuery(QUERYACTION_RefreshAll);
		}
	...
	{
		`log("OnGameTypeChanged: GameTypeClassName not set");
	}
}

`if (`DUMP_FILTERS)
// NOTE: Don't consolidate the really long context-switches; they make it easier to find stuff if looking back
final function DumpGameSearchData(OnlineGameSearch GameSearch)
{
	local int i, j, OutVal;
	local string LogStr;

	`log("Game search details:");
	`log("- MaxSearchResults:"@GameSearch.MaxSearchResults);
	`log("- Query: Name:"@GameSearch.GetStringSettingName(GameSearch.Query.Id)$
		", ColumnName:"@GameSearch.GetStringSettingColumnHeader(GameSearch.Query.Id)$
		", ValueName:"@GameSearch.GetStringSettingValueName(GameSearch.Query.Id, GameSearch.Query.ValueIndex)$
		", AdvertisementType:"@GetEnum(Enum'EOnlineDataAdvertisementType', GameSearch.Query.AdvertisementType));
	`log("- bIsLanQuery:"@GameSearch.bIsLanQuery);
	`log("- bUsesArbitration:"@GameSearch.bUsesArbitration);
	`log("- GameSettingsClass:"@GameSearch.GameSettingsClass);

	`log("- NamedProperties:");

	for (i=0; i<GameSearch.NamedProperties.Length; ++i)
		`log("--"@i$": Name:"@GameSearch.NamedProperties[i].ObjectPropertyName$", Value:"@GameSearch.NamedProperties[i].ObjectPropertyValue);

	`log("- FilterQuery:");

	`log("-- OrClauses:");

	for (i=0; i<GameSearch.FilterQuery.OrClauses.Length; ++i)
	{
		`log("---"@i$": OrParams");

		for (j=0; j<GameSearch.FilterQuery.OrClauses[i].OrParams.Length; ++j)
		{
			LogStr = "----"@j$": Name";

			if (GameSearch.FilterQuery.OrClauses[i].OrParams[j].EntryType == OGSET_Property)
			{
				LogStr @= "(property):"@GameSearch.GetPropertyName(GameSearch.FilterQuery.OrClauses[i].OrParams[j].EntryId);
				LogStr @= "(p"$GameSearch.FilterQuery.OrClauses[i].OrParams[j].EntryId$")";

				LogStr $= ", ColumnName:"@GameSearch.GetPropertyColumnHeader(
								GameSearch.FilterQuery.OrClauses[i].OrParams[j].EntryId);
				LogStr $= ", Value:"@GameSearch.GetPropertyAsString(GameSearch.FilterQuery.OrClauses[i].OrParams[j].EntryId);
			}
			else if (GameSearch.FilterQuery.OrClauses[i].OrParams[j].EntryType == OGSET_LocalizedSetting)
			{
				LogStr @= "(localized):"@GameSearch.GetStringSettingName(GameSearch.FilterQuery.OrClauses[i].OrParams[j].EntryId);
				LogStr @= "(s"$GameSearch.FilterQuery.OrClauses[i].OrParams[j].EntryId$")";

				LogStr $= ", ColumnName:"@GameSearch.GetStringSettingColumnHeader(
								GameSearch.FilterQuery.OrClauses[i].OrParams[j].EntryId);

				if (GameSearch.GetStringSettingValue(GameSearch.FilterQuery.OrClauses[i].OrParams[j].EntryId, OutVal))
					LogStr $= ", Value:"@OutVal;
				else
					LogStr $= ", Value: Unknown (failed to retrieve value)";
			}
			else // if (GameSearch.FilterQuery.OrClauses[i].OrParams[j].EntryType == OGSET_ObjectProperty)
			{
				LogStr @= "(object)"@GameSearch.FilterQuery.OrClauses[i].OrParams[j].ObjectPropertyName;
				LogStr $= ", Value:"@GameSearch.GetPropertyAsStringByName(
								GameSearch.FilterQuery.OrClauses[i].OrParams[j].ObjectPropertyName);
			}


			LogStr $= ", SortType:"@GetEnum(Enum'EOnlineGameSearchComparisonType',
							GameSearch.FilterQuery.OrClauses[i].OrParams[j].ComparisonType);
	...
	for (i=0; i<GameSearch.FilterQuery.SortClauses.Length; ++i)
	{
		LogStr = "---"@j$": Name";

		if (GameSearch.FilterQuery.SortClauses[i].EntryType == OGSET_Property)
		{
			LogStr @= "(property):"@GameSearch.GetPropertyName(GameSearch.FilterQuery.SortClauses[i].EntryId);
			LogStr @= "(p"$GameSearch.FilterQuery.SortClauses[i].EntryId$")";

			LogStr $= ", ColumnName:"@GameSearch.GetPropertyColumnHeader(GameSearch.FilterQuery.SortClauses[i].EntryId);
			LogStr $= ", Value:"@GameSearch.GetPropertyAsString(GameSearch.FilterQuery.SortClauses[i].EntryId);
		}
		else if (GameSearch.FilterQuery.SortClauses[i].EntryType == OGSET_LocalizedSetting)
		{
			LogStr @= "(localized):"@GameSearch.GetStringSettingName(GameSearch.FilterQuery.SortClauses[i].EntryId);
			LogStr @= "(s"$GameSearch.FilterQuery.SortClauses[i].EntryId$")";

			LogStr $= ", ColumnName:"@GameSearch.GetStringSettingColumnHeader(GameSearch.FilterQuery.SortClauses[i].EntryId);

			if (GameSearch.GetStringSettingValue(GameSearch.FilterQuery.SortClauses[i].EntryId, OutVal))
				LogStr $= ", Value:"@OutVal;
			else
				LogStr $= ", Value: Unknown (failed to retrieve value)";
		}
		else // if (GameSearch.FilterQuery.SortClauses[i].EntryType == OGSET_ObjectProperty)
		{
			LogStr @= "(object)"@GameSearch.FilterQuery.SortClauses[i].ObjectPropertyName;
			LogStr $= ", Value:"@GameSearch.GetPropertyAsStringByName(GameSearch.FilterQuery.SortClauses[i].ObjectPropertyName);
		}


		LogStr $= ", SortType:"@GetEnum(Enum'EOnlineGameSearchSortType',GameSearch.FilterQuery.SortClauses[i].SortType);

		`log(LogStr);
	}

	`log("- AdditionalSearchCriteria:"@GameSearch.AdditionalSearchCriteria);
	`log("- PingBucketSize:"@GameSearch.PingBucketSize);
}
`endif
...
DefaultProperties
{
	...
	SubWidgetBindings.Add((WidgetName="header5",WidgetClass=class'GFxClikWidget'))
	SubWidgetBindings.Add((WidgetName="browserbtn_fliter",WidgetClass=class'GFxClikWidget'))
	SubWidgetBindings.Add((WidgetName="browserbtn_refresh",WidgetClass=class'GFxClikWidget'))
	SubWidgetBindings.Add((WidgetName="btn_back",WidgetClass=class'GFxClikWidget'))
